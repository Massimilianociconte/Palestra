...
export class FirestoreService {
  ...
  async syncToCloud() {
    ...
    try {
      ...
      const localBodyStats = JSON.parse(
  ...
  async loadFromCloud() {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        if (data.photos)
          localStorage.setItem("ironflow_photos", JSON.stringify(data.photos)); // Load photos

        // Merge AI Plan History: combine local and cloud, remove duplicates, sort by date
        const cloudAiHistory = data.aiPlanHistory || [];
        const localAiHistory = JSON.parse(
          localStorage.getItem("ironflow_ai_plan_history") || "[]",
        );
        const mergedAiHistory = this.mergeAiPlanHistory(
          localAiHistory,
          cloudAiHistory,
        );
        localStorage.setItem(
  ...
  // Merge Workout Logs from local and cloud
  // Combines both arrays, removes duplicates based on ID, sorts by date (newest first)
  // This is CRITICAL to prevent losing locally saved workouts that haven't synced to cloud yet
  mergeLogs(localLogs, cloudLogs) {
    const combined = [...(localLogs || []), ...(cloudLogs || [])];

    // Create a map to deduplicate by ID
    // If two logs have the same ID, prefer the one with more data (local is usually more complete)
    const uniqueMap = new Map();
    combined.forEach((log) => {
      // Use ID as primary key, fallback to date+workoutName combination for legacy logs without ID
      const key = log.id ? String(log.id) : `${log.date}_${log.workoutName}`;

      if (!uniqueMap.has(key)) {
        uniqueMap.set(key, log);
      } else {
        // If duplicate found, keep the one with more exercises or more complete data
        const existing = uniqueMap.get(key);
        const existingExCount = existing.exercises?.length || 0;
        const newExCount = log.exercises?.length || 0;

        // Prefer log with more exercises, or if equal, the one with wellness data
        if (
          newExCount > existingExCount ||
          (newExCount === existingExCount && log.wellness && !existing.wellness)
        ) {
          uniqueMap.set(key, log);
        }
      }
  ...
  async createSharedWorkout(workoutData) {
    try {
      ...
      if (
        !cleanData.name ||
        typeof cleanData.name !== "string" ||
        cleanData.name.length === 0
      ) {
      ...
      throw new Error(
        `Impossibile creare link di condivisione: ${error.message}`,
      );
    }
  }
